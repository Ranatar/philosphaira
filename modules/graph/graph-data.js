...
function getConceptConnections(conceptId) {
      ...
      // а не длины базы.
      // Порядок уже задан при построении указателя (rebuildIndexes), и
      // сортировать здесь заново значило бы платить за это на каждом из
      // семи вызовов, часть которых в циклах по всем узлам.
...
function updateLinkOnGraph() {
      // Тип, вес и взаимность меняют вид связи (полосу попадания выбор
      // считает на ходу). Признак годности слоя опирается на положения,
      // счёт и ссылки на наборы — ничего из этого правка не трогает,
      // поэтому сброс явный.
      linkLayer.key = null;
      requestDraw();
    }
