...
gfxNode.on("mouseover", function(event, d) {
      ...
      S.tooltipTimeout = setTimeout(() => {
        ...
        tooltip
          .style("opacity", 1)
          .html(`<strong>${labelWithAuthor(d)}</strong><br/>${descriptionHtml(d.description)}<br/><em>${d.concept}</em>${simNote}`)
...
gfxLink.on("mouseover", function(event, d) {
      ...
      if (selectedEdges.has(d) && d.description) {
        tooltipContent += `<div class="tooltip-description">${descriptionHtml(d.description)}</div>`;
      }
