      ...
      const nodeBlock = (node, i, role) => {
        ...
        <div class="path-node-full-description" id="node-desc-${i}">
          ...
          <div class="philosopher-tag" style="background: ${color}; color: ${getContrastColor(color)}"
               data-act-click="open-universal-modal-15" data-a1="${node.concept}"
               data-tip="Открыть окно философа">
            ${node.concept}${phil ? ' · ' + phil.years : ''}
          </div>
          <p><strong>Описание:</strong> ${descriptionHtml(node.extendedDescription) || 'Описание отсутствует'}</p>
        </div>
      `;
      };
          ...
          html += `
            <div class="path-description-item">
              <div class="path-description-header path-open"
                   data-act-click="open-universal-modal-16" data-a1="${src}" data-a2="${tgt}"
              ...
              ${link.description ? `
                <div class="path-description-text">${descriptionHtml(link.description)}</div>
              ` : `
