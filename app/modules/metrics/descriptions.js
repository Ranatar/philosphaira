...
const metricDescriptions = {
      ...
      'closest-pairs': {
        ...
        description: "Наиболее схожие пары по одной из четырёх мер: профиль метрик, структура связей, типы связей, место в сети",
        interpretation: () => "Профильная близость находит функциональные аналоги: понятия, играющие в своих системах одну роль, даже если между ними нет ни одной связи. Структурная — понятия, окружённые одними и теми же соседями. Близость по типам связей — понятия, необычные в одних и тех же отношениях: обе, скажем, насыщены «условием» и «ограничением» больше, чем принято в базе. Близость по месту в сети — сходную топологическую роль (узел ядра, мост, плотное гнездо, периферия); о родстве содержания она не говорит. Меры почти независимы — попарные ранговые корреляции по парам связных концепций не выше 0,3, — поэтому списки заметно расходятся, и расхождение содержательно.",
        usage: () => "Поиск неочевидных параллелей между традициями. Фильтр связности обязателен: у малосвязных концептов почти все метрики нулевые, профили выходят одинаковыми, и без порога верх списка занимают не родственные, а просто бедные данными понятия. Порог по умолчанию равен медианной связности графа. Вид «по месту в сети» при первом выборе досчитывает сетевые метрики — это несколько секунд; считаются они при текущих переключателях весов, направленности и области. Клик по строке открывает пару в подробном сравнении.",
        formula: () => "Профиль: косинус между дважды центрированными векторами 17 метрик. Структура: |A∩B| / |A∪B| по множествам соседей. Типы связей: косинус между дважды центрированными долями типов (входящие и исходящие вместе, с весом связи). Место в сети: косинус между дважды центрированными векторами восьми сетевых метрик — степень, PageRank, посредничество, близость, собственный вектор, взвешенная кластеризация, локальная когезия, rich-club. Двойное центрирование: z-нормировка по каждому признаку через все концепции, затем вычитание среднего по концепции. Отбор: обе концепции со связностью не ниже порога, для структуры дополнительно минимум общих соседей, опционально только пары разных философов или разных традиций"
      },
      'comparison': {
        name: "Сравнение концепций",
        description: "Схожесть двух концепций по четырём мерам: профиль метрик, структура связей, типы связей, место в сети",
        interpretation: () => "Профиль отвечает, сходна ли роль концепций в системе; структура — связаны ли они с одними и теми же понятиями; типы связей — необычны ли они в одних и тех же отношениях; место в сети — сходно ли их положение в топологии графа. Профиль, типы и место в сети — знаковые меры: отрицательное значение значит противоположность, а не просто несходство. Вердикт называет уровень каждой меры и первое подошедшее сочетание: близнецы, аналоги в разных областях графа, одно окружение при разном отношении к нему, совпадение только по месту в сети.",
        usage: () => `Поиск неочевидных параллелей между философскими системами и проверка догадок вида «X у Гуссерля работает так же, как Y у Фреге». Столбики показывают процентиль концепции по каждой метрике профиля среди всех ${S._concepts.length}, поэтому видно не только насколько похожи, но и чем именно. У концепций со связностью ниже медианы профиль и место в сети непоказательны и помечены «н/п».`,
        formula: () => "Профиль: косинус между векторами 17 метрик, дважды центрированными — z-нормировка по каждой метрике, затем вычитание среднего профиля концепции (равносильно корреляции Пирсона); tensionIndex исключён, его корреляция с problemGenerationIndex 0.94. Структура: |A∩B| / |A∪B| по множествам соседей. Типы связей: то же двойное центрирование по долям типов. Место в сети: то же по восьми сетевым метрикам при текущих переключателях. Уровни вердикта: высокое — не ниже 90-го процентиля по всем парам связных концепций, противоположное — не выше 10-го; для структуры высокое — не меньше трёх общих соседей. Слово о месте в сети: периферия, если концепция в среднем ниже обычного на полсигмы, иначе — по сильнейшей составляющей (степень, PageRank, собственный вектор, rich-club — узел ядра; посредничество, близость — мост; кластеризация, когезия — плотное гнездо)"
      },
      'generative': {
