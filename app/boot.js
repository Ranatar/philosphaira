...
export async function boot() {
  ...
  if (layoutFromStore) {
        // ЭНЕРГИЯ ГАСИТСЯ ВМЕСТЕ С ОСТАНОВОМ. Симуляция рождается с alpha = 1,
        // и остановленная до первого тика хранила её: первое перетаскивание
        // (цель 0,3 НИЖЕ единицы) не разогревало граф, а остужало его с полной
        // энергии и перестраивало раскладку целиком. С нуля перетаскивание
        // само подаёт энергию (alphaTarget 0,3) и отдаёт её после отпускания.
