...
const ЖДЁМ = {
  ...
  концепций: 689, связей: 2201, философов: 100, традиций: 25, рубрик: 15, типовСвязей: 21,
...
//   без сброса (подлог)  — тиков 301, медианный сдвиг 1.9 px
{
  ...
  // Порог понижен 2026-09-07 с 20 до 15 px. После захода 10 (32 связи, из них
  // 7 рефлексивных) раскладка стала жёстче, и замер лёг ровно на прежний
  // порог: два прогона подряд дали 19.4 и 20.5 px. Утверждение начало мигать,
  // не сообщая ничего о правке; проверяется здесь сам факт разъезда, а не его
  // величина, и 15 px от него так же далеко, как прежние 20 были от нуля.
  проверить('после правки базы узлы разъезжаются',
    медиана > 15, '>15 px', медиана.toFixed(1) + ' px');
