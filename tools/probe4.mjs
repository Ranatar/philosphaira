...
async function run(label) {
  ...
  // ── 4. содержимое вывезенного PNG ─────────────────────────────────
  {
    ...
    // ОЖИДАНИЕ ПО СОБЫТИЮ, А НЕ ГЛУХИМ СРОКОМ. Прежние 4 с хватало на
    // свободной машине и не хватало под нагрузкой: одностраничный эталон
    // весит 2,19 МБ, и вывоз PNG на нём медленнее. Прибор отвечал «нет» —
    // то есть чужой занятостью, а не свойством страницы. Тот же изъян уже
    // лечили в sweep_all (readme §6): срок стал ПОТОЛКОМ, а не мерой.
    let пн = null;
    for (let i = 0; i < 40 && !пн; i++) {
      await wait(500);
      пн = await page.evaluate(() => {
        const z = window.__вывоз[0];
