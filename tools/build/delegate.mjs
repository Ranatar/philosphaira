...
// ── постоянная оснастка ─────────────────────────────────────────────
const HEAD = '// Сгенерировано tools/build/delegate.mjs — правки вносить туда.\n';
...
if (fs.existsSync(path.join(ROOT, file))) {
  ...
  // Якорь — сам запуск, а не строка моста: мост (installBridge) снят из
  // сборки 2026-09-10, и делегирование обязано ставиться ПЕРЕД boot().
  if (!main.includes('installDelegation()'))
    main = main.replace(/^boot\(\)\.catch/m,
  ...
  if (!fs.readFileSync(mp, 'utf8').includes('installDelegation()')) {
    console.error('delegate: установка делегирования в main.js не появилась — якорь «boot().catch» не найден.');
    process.exit(1);
  }
}
