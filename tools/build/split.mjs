...
//     импорты безопасны.
//  7. Разметка зовёт действия по имени из реестра (delegate.mjs), глобальных
//     имён у приложения нет; мост (bridge.js) снят за ненадобностью.
...
// человек, этот всегда порождён (tools/edit/layout.mjs). Лежит здесь потому, что
...
// Моста нет с 2026-09-10. Он выставлял в window то, что звала разметка
// встроенными обработчиками; после делегирования (delegate.mjs static/dyn)
// таких нет — bridge_debt.py держит «атрибутов on* 0». Прежде мост
// строился здесь и снимался целиком шагом unbridge.mjs; пара «построить —
// снести» заменена на «не строить». Если в разметке появится атрибут,
// который делегирование не переведёт, страница упадёт с ReferenceError —
// громко, и bridge_debt покраснеет до неё.
