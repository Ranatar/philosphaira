...
//   globals_map_v3.md, map_tree.md — читаемые виды двух карт. Их не читает
//                        ни одна программа, и до 2026-09-10 их не проверял
//                        никто: лежали на неделю старше своих JSON.
//                        Сверяются с выводом map_to_md.py / map_tree_to_md.py
//                        без строки «Составлено …».
//
// Отметка времени в сравнении не участвует: она различается всегда.
import fs from 'node:fs';
import { ДЕРЕВО, ИСХОДНИК, КОРЕНЬ, программа } from '../paths.mjs';
...
for (const к of карты) {
  ...
  try {
    execFileSync('node', [программа(к.как[0]), ...к.как[1]], { stdio: 'pipe' });
  } catch (e) {
...
// значит после последней правки базы забыли `node tools/edit/layout.mjs`, и
// страница будет каждый раз считать раскладку заново, теряя двадцать секунд.
{
  ...
  if (!fs.existsSync(файл)) {
    ...
    if (свой === null) {
      ...
      console.log(`✗ data/nodePositions.json — УСТАРЕЛ (в наборе ${набор.fingerprint}, у базы ${свой}); пересчитать: node tools/edit/layout.mjs`);
      плохо++;
    }
  }
}

// читаемые виды карт — python-программы по свежим JSON из набора
{
  const безДаты = т => т.replace(/Составлено [^\n]*/g, 'Составлено …');
  for (const [имя, имяПрограммы, вход] of [
    ['mapping/globals_map_v3.md', 'map_to_md.py', 'mapping/globals_map_v3.json'],
    ['mapping/map_tree.md', 'map_tree_to_md.py', 'mapping/map_tree.json'],
  ]) {
    const старый = есть(имя);
    const новый = path.join(ВРЕМ, path.basename(имя));
    try {
      execFileSync('python3', [программа(имяПрограммы), path.join(КОРЕНЬ, вход), новый], { stdio: 'pipe' });
    } catch (e) {
      console.log(`✗ ${имя} — пересобрать не удалось: ${String(e.message).split('\n')[0]}`);
      плохо++; continue;
    }
    if (!fs.existsSync(старый)) { console.log(`? ${имя} — НЕ НАЙДЕНА`); плохо++; continue; }
    const свежа = безДаты(fs.readFileSync(старый, 'utf8')) === безДаты(fs.readFileSync(новый, 'utf8'));
    console.log(`${свежа ? '✓' : '✗'} ${имя} — ${свежа ? 'свежая' : 'УСТАРЕЛА (читаемый вид карты; пересобрать: python3 ' + программа(имяПрограммы) + ')'}`);
    if (!свежа) плохо++;
  }
}
...
// спецификация модулей — текстом
{
  ...
  execFileSync('node', [программа('gen_spec2.mjs'), ДЕРЕВО, новый], { stdio: 'pipe' });
  if (!fs.existsSync(старый)) {
