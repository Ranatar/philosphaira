...
window.__snap = function () {
  ...
  // ЛЕГЕНДА: снимаем её БЕЗ того, что уже меряется отдельно. Прежде поле
  // хешировало innerHTML целиком, и в него попадали две гуляющие величины:
  //  - #filterStats («Показано: 453/453 концепций, 1624/1624 связей») — она же
  //    стоит отдельным полем ниже, то есть учитывалась дважды, а её длина
  //    меняется от числа цифр (замер: 94 ↔ 89 знаков);
  //  - тело поиска с находками — оно раздувает легенду с 2 491 до 153 992
  //    знаков и обходится СВОЕЙ областью («найденное в легенде»).
  // Отсюда отпечаток легенды зависел от того, что успел нажать обход, и
  // расходился между машинами при одинаковом поведении. Лечение то же, что
  // применено ниже к виду статистики: не подгонять состояние под прибор, а
  // спросить о неизменном.
  // Показательность сохранена: прибавь или убери фильтр, философа, кнопку,
  // раздел — снимок это увидит.
  var legendSnap = function () {
    var e = document.getElementById('legend');
    if (!e) return 'нет';
    var c = e.cloneNode(true);
    //  - #visualizationControlSection — раздел «визуализируется такая-то
    //    метрика». Он лежит В ЛЕГЕНДЕ и называет ту метрику, на которой
    //    визуализация успела сработать. Кнопка «Визуализировать размером»
    //    появляется в том виде, где метрика досчиталась первой, а это гуляет
    //    (см. «устояться» ниже) — и раздел то показан, то скрыт, то называет
    //    Betweenness, то Eigenvector. Отсюда расхождение сторон на 11 знаков
    //    в 38 видах: имена метрик разной длины. Обработчики самого раздела
    //    при этом обходятся вместе с главной областью и не теряются.
    var убрать = c.querySelectorAll(
      '#filterStats, #searchBody, #visualizationControlSection');
    for (var i = 0; i < убрать.length; i++) убрать[i].remove();
    return window.__hash(c.innerHTML);
  };
  return [
    legendSnap(), g('pathFinder'), g('controls'), g('authButtons'),
...
window.__handlers = function (rootSel) {
  ...
  for (var i = 0; i < all.length; i++) {
    var el = all[i];
    // Кнопки, запускающие тяжёлый досчёт с живым процентом, обход не жмёт:
    // итог нажатия зависел бы от того, успел ли расчёт к снимку. Их ход и
    // результат проверяет progress_probe (data-sweep-skip ставит исходник).
    if (el.closest && el.closest('[data-sweep-skip]')) continue;
...
async function run(pageName, part) {
  ...
  for (const v of VIEWS) {
    ...
    // СОСТОЯНИЕ ПЕРЕД СНИМКОМ — ИЗВЕСТНОЕ, А НЕ СЛУЧАЙНОЕ. Обход жмёт в том
    // числе кнопку закрытия, окно захлопывается, и ensureStats открывает его
    // заново — но подложка (#modalOverlay) переходит в видимое не мгновенно.
    // Два прогона ОДНОЙ стороны разошлись ровно этим: у вида critical-power
    // поле 16 было то block, то none. Возвращаем окно и даём подложке
    // устояться до снимка, а не после.
    await ensureStats();
    await wait(300);
    await record('вид ' + v);
  }
  ...
  // МНОГОПОЛЬЗОВАТЕЛЬСКУЮ ЧАСТЬ ЭТОТ ПРИБОР ОБОЙТИ НЕ МОЖЕТ, и это не
  // недосмотр. Обход идёт по странице, отданной `tools/serve.py` — сервера
  // API за ней НЕТ. Значит `serverMode` ложен, панели правок, людей и
  // колокола не поднимаются вовсе, и добавленные было области давали ровно
  // нули: 0, 0, 0, 0. Проба, всегда показывающая ноль, хуже отсутствующей —
  // она создаёт видимость присмотра.
  //
  // Многопользовательскую часть стережёт `server/probes/page_probe.mjs`:
  // он поднимает НАСТОЯЩИЙ сервер и смотрит страницу его глазами. Туда и
  // надо добавлять покрытие панелей.
