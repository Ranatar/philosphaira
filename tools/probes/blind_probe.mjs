...
// пересчётов и зовётся явно: `node tools/probes/blind_probe.mjs run <страница>
// <файл> --медленно`.
...
if (команда !== 'run') {
  console.error('употребление: node tools/probes/blind_probe.mjs run <страница> <файл.json> [--медленно]');
  process.exit(2);
}
