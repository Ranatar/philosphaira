...
// прибор этого не спрашивал.
{
  ...
  // ФИЛОСОФЫ У СВЯЗЕЙ. «Идея блага → Единое» не говорит, между чьими
  // системами связь, — а это главный вопрос к связи.
  проверить('у строки связи названы философы обоих концов',
    строки.уСвязиЕстьПодпись === true, true, строки.уСвязиЕстьПодпись);
...
// не понимал, почему концепции Спинозы и Локка идут вперемешку.
{
  const порядок = await page.evaluate(() => {
    ...
    // 2. ВНУТРЕННИЕ СВЯЗИ ВПЕРЕДИ ВНЕШНИХ — у каждой концепции.
    let проверено = 0;
    for (const n of A.DATA.nodes.slice(0, 60)) {
      const св = A.getConceptConnections(n.id);
      const свой = x => { const c = A.conceptById.get(x.id || x); return c ? c.concept : ''; };
      const внутр = св.map(l => свой(l.source) === свой(l.target));
      const последнийВнутр = внутр.lastIndexOf(true);
      const первыйВнешний = внутр.indexOf(false);
      if (последнийВнутр !== -1 && первыйВнешний !== -1 && последнийВнутр > первыйВнешний) {
        беда.push('внешняя перед внутренней у ' + n.id);
      }
      проверено++;
    }
  ...
  проверить('внутренние связи идут перед внешними',
    !порядок.беда.some(б => б.startsWith('внешняя')), 0,
    порядок.беда.filter(б => б.startsWith('внешняя'))[0] || 0);
