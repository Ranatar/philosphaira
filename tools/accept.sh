...
# PUPPETEER здесь больше не перекрывается: умолчание стоит в tools/paths.mjs,
# и оно то же, что прежде задавала эта строка (свой puppeteer-core проекта).
# Два места с одним знанием однажды разошлись — приборы руками и из приёмки
# шли через разные выпуски. Заданная снаружи переменная проходит как есть.
...
if [ "$WHAT" != "--только-приложение" ]; then
  ...
  if [ -z "${DATABASE_URL:-}" ]; then
    ...
    # ОДИН прогон. Прежде их было два подряд — первый ради печати, второй
    # ради кода возврата, — то есть вся серверная приёмка (21 проба, в том
    # числе две с браузером) шла дважды, и вердикт выносился по второму
    # прогону, которого никто не видел. Вывод пишется в файл, код берётся
    # из того же прогона.
    bash server/accept.sh > "$SNAPS/server.log" 2>&1; SERVER_RC=$?
    grep -E "^══|не сошлось|ПРИЁМКА" "$SNAPS/server.log"
    verdict "приёмка сервера" $SERVER_RC
  fi
fi
