...
if [ "$WHAT" != "--только-сервер" ]; then
  ...
  # Ход долгих расчётов — отдельным прибором: снимки вычёркивают живой
  # процент (data-live-progress), обход не жмёт тяжёлые кнопки
  # (data-sweep-skip), а что проценты идут и кончаются результатом,
  # проверяет только он.
  serve_up; node "$(tool progress_probe.mjs)" 2>&1 | tail -1 | grep -q "не сошлось 0"; verdict "progress_probe" $?
  # Ссылки — проверка кругом: состояние, адрес, чистая вкладка по нему,
  # сличение отпечатков. Эталона у прибора нет: он сверяет страницу с нею же.
  serve_up; node "$(tool link_probe.mjs)" 2>&1 | tail -1 | grep -q "не сошлось 0"; verdict "link_probe" $?
  # Нарисованное против выбранного: кадр записывается (arc/moveTo/lineTo/
  # fill/stroke), геометрия судится по нему самому, выбор узла и связи — по
  # нему же; плюс PNG- и SVG-вывоз той же меркой. Восемь состояний экрана.
  serve_up; node "$(tool hit_probe.mjs)" 2>&1 | tail -1 | grep -q "не сошлось 0"; verdict "hit_probe" $?
