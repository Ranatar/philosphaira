#!/usr/bin/env node
// РАСКЛАДКА, ПОСЧИТАННАЯ ЗАРАНЕЕ — НЕ шаг цепочки сборки, а круг вокруг неё.
//
// Прежде шапка звала программу «девятым шагом цепочки», а readme §5
// объяснял, почему шагом она быть не может: раскладка считается по
// СОБРАННОМУ дереву и пишется в ИСХОДНИК, поэтому нужен полный круг
// «собрать → посчитать → собрать». Верно второе. Запускать после всякой
// правки, задевшей состав концепций или связей; забыть молча не даст
// maps_fresh.mjs — он сверяет отпечаток базы с отпечатком в наборе.
//
