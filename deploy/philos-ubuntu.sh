...
установить() {
  ...
  if node -v 2>/dev/null | grep -q '^v2[2-9]'; then
    ...
    как_root apt-get install -y nodejs > /dev/null
    годно "поставлен $(node -v)"
  fi

  шаг "PostgreSQL и мелочь"
  как_root apt-get install -y postgresql python3 python3-venv curl unzip git > /dev/null
  годно "$(psql --version)"
  ...
  как_root apt-get install -y libnss3 libatk1.0-0 libatk-bridge2.0-0 libcups2 \
    libdrm2 libxkbcommon0 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 \
    libgbm1 libpango-1.0-0 libasound2t64 > /dev/null
  годно "поставлены"
