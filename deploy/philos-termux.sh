...
наполнить() {
  ...
  if ( cd "$ROOT/server" && npm run graph:import ) > "$HOME/.philos-import.log" 2>&1; then
    годно "семь наборов внесены: шесть в граф, седьмой — раскладка из семени"
  elif grep -q "уже есть живые сущности" "$HOME/.philos-import.log"; then
    годно "граф уже не пуст — семя не трогаю (так и задумано)"
  else
    стоп "перенос графа не прошёл: $(tail -3 "$HOME/.philos-import.log")"
  fi
  ...
  шаг "раскладка"
  LAYOUTS=$(psql "$DATABASE_URL" -tAc "SELECT count(*) FROM graph_layout" 2>/dev/null | tr -d ' ')
  if [ -n "$LAYOUTS" ] && [ "$LAYOUTS" != "0" ]; then
    годно "раскладка на месте (строк в истории: $LAYOUTS)"
  else
    беда "раскладки нет — страница будет считать её сама"
  fi
  ...
  ( cd "$ROOT/server" \
    && BOOTSTRAP_ADMIN_LOGIN="${BOOTSTRAP_ADMIN_LOGIN:-admin}" \
       ...
       npm run bootstrap-admin ) > "$HOME/.philos-admin.log" 2>&1
  # Все четыре строки, а не две, и логин ОТДЕЛЬНОЙ строкой — как в
  # Ubuntu-близнеце: первая строка bootstrap-admin называет логин, и обрезка
  # до двух её съедала. На повторе («ничего не сделано») логин не печатается:
  # заведён он был прежде и мог быть другим.
  if grep -q "ничего не сделано" "$HOME/.philos-admin.log"; then
    годно "люди в базе уже есть — администратора не завожу (так и задумано)"
  else
    grep -v '^>' "$HOME/.philos-admin.log" | grep -v '^$' | tail -4
    printf "\n"
    годно "ЛОГИН ДЛЯ ВХОДА: ${BOOTSTRAP_ADMIN_LOGIN:-admin}   (пароль — тот, что вы сейчас задали)"
  fi
}
...
состояние() {
  ...
  # ИМЯ ЛАТИНИЦЕЙ: `копится=…` bash не принимает — присваивание падало с
  # «command not found», а "$копится" разворачивалось в буквальный текст, и
  # `состояние` ПРИ КАЖДОМ ВЫЗОВЕ печатало ложное «не разослано: $копится».
  # И БАЗА НАЗВАНА: `psql` без неё идёт в базу с именем пользователя Termux,
  # которой нет, — счёт молчал и по этой причине тоже.
  PENDING=$(psql "${DATABASE_URL:-}" -tAc "SELECT count(*) FROM outbox WHERE delivered_at IS NULL" \
    2>/dev/null | tr -d ' ')
  [ -n "$PENDING" ] && [ "$PENDING" != "0" ] \
...
приёмка() {
  ...
  page_probe и live_graph_probe не пойдут: они поднимают Chrome, которого
  в Termux нет. Остальные девятнадцать проб сервера из двадцати одной — идут.
ТЕКСТ
