...
export const ОПЦИИ = Object.freeze({
...
export const hashPassword = пароль => argon2.hash(пароль, ОПЦИИ);

/** Никогда не бросает: неверный пароль и битый хеш — оба «не сошлось». */
export const verifyPassword = (хеш, пароль) =>
  argon2.verify(хеш, пароль, ОПЦИИ).catch(() => false);
...
// дешевле настоящей.
export const ХОЛОСТОЙ_ХЕШ = await argon2.hash(
  'нет такого пользователя:' + Math.random(), ОПЦИИ);

export const МИН_ДЛИНА = 12;
...
export function assertPasswordPolicy(пароль) {
  if (typeof пароль !== 'string' || пароль.length < МИН_ДЛИНА) {
    throw new Forbidden(`Пароль короче ${МИН_ДЛИНА} знаков`);
  }
  ...
  if (ЧАСТЫЕ.has(пароль.toLowerCase())) {
