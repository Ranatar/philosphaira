...
try {
  ...
  // ── 7а. ГОСТЬ СОКЕТА НЕ ПОЛУЧАЕТ (F-5, п. 18) ───────────────────────────
  //
  // Это РЕШЕНИЕ, а не упущение: гость видит снимок, чужие правки — после
  // перезагрузки. По сокету идут не только правки графа, но и личные
  // извещения, и пока они не разведены, всякий сокет несёт то, что гостю
  // знать незачем. Решение объявлено в `ws/manager.js` и в инструкции — а
  // здесь стережётся, чтобы не переменилось молча.
  let гость = 'подключился';
  try {
    const ws = подключиться(8801, null);
    await ws.открылся;
  } catch { гость = 'отказ'; }
  проверить('ГОСТЬ (без сессии вовсе) СОКЕТА НЕ ПОЛУЧАЕТ', гость === 'отказ',
    'отказ', гость);

  let сЧужимПризнаком = 'подключился';
  try {
    const ws = подключиться(8801, 'выдуманный-признак-сеанса');
    await ws.открылся;
  } catch { сЧужимПризнаком = 'отказ'; }
  проверить('и с выдуманным признаком сеанса тоже', сЧужимПризнаком === 'отказ',
    'отказ', сЧужимПризнаком);
