...
// ── матрица прав сверяется с напечатанной ────────────────────────────────
//
// ЛЕЖИТ В `doc/server-design.md`, А НЕ В README: 27.08 инструкция оператору
// отделена от разбора устройства (п. 15 ревизии). Прибор поймал переезд в
// тот же час — «строк матрицы 0 вместо 16», — и это ровно та польза, ради
// которой он сверяет НАПЕЧАТАННОЕ с действующим, а не верит на слово.
const readme = fs.readFileSync(
  path.join(КОРЕНЬ, '..', 'doc', 'server-design.md'), 'utf8');
