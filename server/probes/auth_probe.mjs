...
try {
  ...
  // ВХОД ИМЕНЕМ, А НЕ ТОЛЬКО ПОЧТОЙ. Форма подписана «Логин или почта», и
  // служба обязана принимать оба. Прежде она искала ТОЛЬКО по почте, а
  // findByUsername была написана и не звалась: подпись обещала одно, служба
  // делала другое, и не проверял этого никто.
  const поИмени = await login(pool, { login: 'иван', password: ПАРОЛЬ });
  проверить('вход по ИМЕНИ даёт токен', !!поИмени.токен, 'токен', поИмени.токен ? 'есть' : 'нет');
  проверить('вход по имени — тот же человек',
    поИмени.user.userId === вход.user.userId, вход.user.userId, поИмени.user.userId);

  const поПочтеНовымПолем = await login(pool, { login: 'ivan@e.рф', password: ПАРОЛЬ });
  проверить('вход по ПОЧТЕ через то же поле', !!поПочтеНовымПолем.токен,
    'токен', поПочтеНовымПолем.токен ? 'есть' : 'нет');

  // Регистр не должен значить ничего: обе выборки идут через lower().
  const инойРегистр = await login(pool, { login: 'ИВАН', password: ПАРОЛЬ });
  проверить('имя нечувствительно к регистру', !!инойРегистр.токен,
    'токен', инойРегистр.токен ? 'есть' : 'нет');

  // СТАРОЕ ИМЯ ДОВОДА ЖИВО: его шлют пробы и всё, что писалось раньше.
  const староеПоле = await login(pool, { email: 'ivan@e.рф', password: ПАРОЛЬ });
  проверить('довод email принимается по-прежнему', !!староеПоле.токен,
    'токен', староеПоле.токен ? 'есть' : 'нет');

  const неизвестноеИмя = await отказ(() => login(pool, { login: 'такого-нет', password: ПАРОЛЬ }));
  ...
  проверить('неизвестное ИМЯ отвечает так же, как неизвестный адрес',
    неизвестноеИмя === нетТакого, нетТакого, неизвестноеИмя);
