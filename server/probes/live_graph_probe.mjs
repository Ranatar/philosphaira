...
try {
  ...
  // КОДА СЕРВЕРА расширение не касалось: правился только граф, но сервер
  // держит его собственную копию, и без повторного `npm run graph:import`
  // эти числа расходятся с приложением молча.
  проверить('узлы на месте после перехода', до.узлов === 718, 718, до.узлов);
  проверить('связи на месте после перехода', до.связей === 2720, 2720, до.связей);
