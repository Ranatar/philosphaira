...
// ── 9. политика версий одна на весь проект ───────────────────────────────
// Закреплено ТОЧНО, без ^ и ~: приёмка обязана воспроизводиться, а сервер,
// где express подтянулся на минорную версию выше, проходил бы её иначе, чем
// тот, на котором её учреждали. Прежде здесь было смешение — pg точно,
// прочие с ^, — и это был недосмотр, а не решение.
{
  const пакет = JSON.parse(fs.readFileSync(path.join(КОРЕНЬ, 'package.json'), 'utf8'));
  const все = { ...(пакет.dependencies ?? {}), ...(пакет.devDependencies ?? {}) };
  const плавающие = Object.entries(все)
    .filter(([, deleted]) => /^[\^~]/.test(String(deleted))).map(([и]) => и);
  проверить('версии зависимостей закреплены ТОЧНО, без ^ и ~',
    плавающие.length === 0, 0, плавающие.join(', ') || 'нет');
}
