...
import { задеваетРаскладку, полнаяРаскладка, дорасклад, расхождение } from '../src/graph/layout.js';
...
// Раскладка зависит ТОЛЬКО от множества узлов и пар «источник — цель».
{
  ...
  for (const [имя, changes, ждём] of случаи)
    утв(`задевает раскладку — ${имя}`, задеваетРаскладку(changes) === ждём, ждём, задеваетРаскладку(changes));
  утв('пустой коммит раскладку не задевает', задеваетРаскладку([]) === false, false, задеваетРаскладку([]));
}
...
// ── 2. дорасклад против полного пересчёта ──────────────────────────
// ГЛАВНОЕ ЧИСЛО ЗАХОДА. Полный пересчёт после ОДНОЙ добавленной связи
// перетасовывает весь граф; дорасклад двигает только окрестность правки.
{
  ...
  const основа = полнаяРаскладка(граф);

  // контроль: без правки пересчёт из тех же координат не двигает ничего
  const { позиции: контроль } = дорасклад(граф, основа);
  const мК = расхождение(основа, контроль);
  утв('дорасклад без правки почти не двигает', мК.медиана < 15, '<15 px', `${мК.медиана} px`);
  ...
  const { позиции: тёплый } = дорасклад(свежий, основа);
  const мТ = расхождение(основа, тёплый);
  const холодный = полнаяРаскладка(свежий);
  const мХ = расхождение(основа, холодный);

  утв('дорасклад держит картину', мТ.медиана < 40, '<40 px', `${мТ.медиана} px`);
  утв('полный пересчёт картину теряет', мХ.медиана > 60, '>60 px', `${мХ.медиана} px`);
  утв('дорасклад заметно бережнее пересчёта',
  ...
  const { позиции: сУзлом, новых } = дорасклад(сНовым, основа);
  ...
  const { позиции: ещёРаз } = дорасклад(сНовым, основа);
  const мП = расхождение(сУзлом, ещёРаз);
  утв('дорасклад повторим', мП.медиана === 0, '0 px', `${мП.медиана} px`);
}
...
try {
  ...
  const первая = полнаяРаскладка(граф);
  ...
  утв('дорасклад в транзакции держит картину',
  ...
  утв('откат вернул прежние координаты',
    расхождение(цельОтката.позиции, послеОтката.позиции).медиана === 0, '0 px',
    расхождение(цельОтката.позиции, послеОтката.позиции).медиана + ' px');
  ...
  // обязаны лечь В ТОЙ ЖЕ транзакции, что и раскладка.
  {
    ...
    const новыеПозиции = полнаяРаскладка(граф);
    const мера3 = расхождение(послеОтката.позиции, новыеПозиции);
    const id3 = await saveLayout(client, {
      версияГрафа: версия3, род: 'full', изЧего: послеОтката.id,
      позиции: новыеПозиции, расхождение: мера3,
    });
