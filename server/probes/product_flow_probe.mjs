...
import { resetCounter } from '../src/auth/throttle.js';
...
try {
  ...
  const сИсточником = await автор.зов('/api/commits', { method: 'POST',
    body: { message: 'проба: правка с указанием источника',
            changes: [{ action: 'edit', kind: 'concept', entityId: 'logos',
                        // СТРОКА ВМЕСТЕ С СОСТОЯНИЕМ. Проба старше поля
                        // provenanceStatus (27.08) и подавала одну строку при
                        // «не указано» — несогласие, которое правило запрещает
                        // и сервер до 26.09.2026 пропускал (суд только по
                        // правке); теперь суд по итогу, и так подаёт окно.
  ...
  // ── 11ж-бис. ТРИ ПРАВА НА ЗАМЕРЫ ──────────────────────────────────────
  //
  // Прежде все пять ходов службы спрашивали VIEW_GRAPH: записать и прочесть
  // ЧУЖИЕ замеры мог любой вошедший, а удалить — никто, включая
  // администратора. Здесь проверяется, что каждое из трёх действий отмерено
  // своей меркой.
  // ПРЕДЕЛ РЕГИСТРАЦИЙ СЧИТАЕТСЯ ПО АДРЕСУ, а все действующие лица пробы
  // приходят с 127.0.0.1 — то есть делят одну пятёрку на всех (REGISTER_LIMIT).
  // Проба стояла ровно на пределе, и шестое лицо роняло не себя, а СЛЕДУЮЩЕЕ:
  // регистрация возвращала отказ без поля data, и падение вылезало
  // «Cannot read properties of undefined» за двести строк от причины.
  // Счёт сбрасывается здесь явно — это не обход заслона, а признание, что
  // предел стоит на людей, а не на подставные лица одной пробы.
  resetCounter('register:::ffff:127.0.0.1');
  resetCounter('register:127.0.0.1');

  const зритель = браузер('зритель_замеров');
  const регЗ = await зритель.зов('/api/auth/register', { method: 'POST',
    body: { username: 'зритель_замеров', email: 'obsviewer@example.invalid',
            password: ПАРОЛЬ } });
  await withTransaction(pool, async client => {
    await setEmailVerified(client, регЗ.тело.data.userId);   // роль остаётся viewer
  });

  const зрительПишет = await зритель.зов('/api/metrics/observations', { method: 'POST',
    body: { metric: 'criticalPowerIndex', formulaVersion: 1, flags: 'x',
            scope: [], values: summary } });
  проверить('ЗРИТЕЛЬ ЗАМЕР НЕ ЗАПИШЕТ', зрительПишет.код >= 400, '4xx', зрительПишет.код);

  const зрительВидит = await зритель.зов('/api/metrics/observations?limit=50');
  проверить('зритель видит список — но ПУСТОЙ: чужих ему не дано',
    зрительВидит.код === 200 && (зрительВидит.тело?.data?.items?.length ?? -1) === 0,
    '200 и 0 записей',
    `${зрительВидит.код} и ${зрительВидит.тело?.data?.items?.length}`);

  // Чужой замер неотличим от несуществующего — иначе перебор по
  // идентификаторам сообщал бы, какие замеры на свете есть и у кого.
  const зрительВЛоб = await зритель.зов('/api/metrics/observations/' + замерId1);
  проверить('чужой замер по прямой ссылке НЕ ОТДАЁТСЯ',
    зрительВЛоб.код >= 400, '4xx', зрительВЛоб.код);

  // Автор (редактор) своих видит.
  const свои = await автор.зов('/api/metrics/observations?limit=50');
  проверить('автор видит СВОИ замеры',
    (свои.тело?.data?.items?.length ?? 0) > 0, '>0', свои.тело?.data?.items?.length);

  // Рецензент — администратор со вторым шагом: видит чужие и может удалить.
  const чужиеРецензенту = await рец.зов('/api/metrics/observations?limit=50');
  проверить('МОДЕРАТОР И ВЫШЕ видит чужие замеры',
    (чужиеРецензенту.тело?.data?.items?.length ?? 0) > 0, '>0',
    чужиеРецензенту.тело?.data?.items?.length);
  проверить('и у чужого замера видно имя автора',
    !!(чужиеРецензенту.тело?.data?.items ?? []).find(o => o.authorName),
    'имя есть', 'имени нет');

  const авторУдаляет = await автор.зов('/api/metrics/observations/' + замерId2,
    { method: 'DELETE' });
  проверить('РЕДАКТОР УДАЛИТЬ НЕ МОЖЕТ даже свой',
    авторУдаляет.код >= 400, '4xx', авторУдаляет.код);
  ...
  // ── 11и. СОСТОЯНИЕ ПРОИСХОЖДЕНИЯ (F-2, п. 2 ревизии) ─────────────────
  //
  // Одна строка не отвечала, ПОЧЕМУ источника нет. Теперь состояние сказано
  // прямо — и стережётся, чтобы не разошлось со строкой.
  const сСостоянием = await автор.зов('/api/commits', { method: 'POST',
    body: { message: 'проба: источник с состоянием',
            changes: [{ action: 'edit', kind: 'concept', entityId: 'nous',
                        fields: {
                          provenance: { base: null, next: 'Плотин, Эннеады V.1' },
                          provenanceStatus: { base: null, next: 'sourced' } } }] } });
  проверить('правка с состоянием происхождения принимается',
    сСостоянием.код === 202, 202,
    `${сСостоянием.код} ${JSON.stringify(сСостоянием.тело?.error ?? '').slice(0, 60)}`);

  // ГЛАВНЫЕ УТВЕРЖДЕНИЯ: состояние не смеет расходиться со строкой.
  const пустоНоЕсть = await автор.зов('/api/commits', { method: 'POST',
    body: { message: 'проба: sourced без ссылки',
            changes: [{ action: 'edit', kind: 'concept', entityId: 'logos',
                        fields: {
                          provenance: { base: null, next: '  ' },
                          provenanceStatus: { base: null, next: 'sourced' } } }] } });
  проверить('«ИСТОЧНИК ЕСТЬ» БЕЗ ССЫЛКИ ОТВЕРГНУТ', пустоНоЕсть.код >= 400,
    '4xx', пустоНоЕсть.код);

  const естьНоНеУказан = await автор.зов('/api/commits', { method: 'POST',
    body: { message: 'проба: unspecified со ссылкой',
            changes: [{ action: 'edit', kind: 'concept', entityId: 'logos',
                        fields: {
                          provenance: { base: null, next: 'Гераклит B1' },
                          provenanceStatus: { base: null, next: 'unspecified' } } }] } });
  проверить('«НЕ УКАЗАНО» ПРИ ЗАПОЛНЕННОЙ ССЫЛКЕ ОТВЕРГНУТО',
    естьНоНеУказан.код >= 400, '4xx', естьНоНеУказан.код);

  const небылица = await автор.зов('/api/commits', { method: 'POST',
    body: { message: 'проба: выдуманное состояние',
            changes: [{ action: 'edit', kind: 'concept', entityId: 'logos',
                        fields: { provenanceStatus: { base: null, next: 'кажется' } } }] } });
  проверить('неизвестное состояние отвергнуто', небылица.код >= 400,
    '4xx', небылица.код);

  // «ИСКАЛИ И НЕ НАШЛИ» — состояние БЕЗ строки, и это законно: оно говорит
  // о ходе работы, а не о предмете.
  const неНашли = await автор.зов('/api/commits', { method: 'POST',
    body: { message: 'проба: искали и не нашли',
            changes: [{ action: 'edit', kind: 'concept', entityId: 'logos',
                        fields: { provenanceStatus:
                          { base: null, next: 'source_not_found' } } }] } });
  проверить('«ИСКАЛИ И НЕ НАШЛИ» принимается без ссылки',
    неНашли.код === 202, 202,
  ...
  const нусПосле = (await гость.зов('/api/graph')).тело?.data?.наборы
    ?.concepts?.find(к => к.id === 'nous') ?? {};
  проверить('состояние дошло до выгрузки',
    нусПосле.provenanceStatus === 'sourced', 'sourced', нусПосле.provenanceStatus);
  проверить('и строка источника вместе с ним',
    /Плотин/.test(String(нусПосле.provenance ?? '')), 'Плотин',
    нусПосле.provenance);

  // Сущность БЕЗ источника не обрастает полем — как и прежде со строкой.
  const безПоля = ((await гость.зов('/api/graph')).тело?.data?.наборы?.concepts ?? [])
    .filter(к => 'provenanceStatus' in к).length;
  проверить('поле состояния есть ТОЛЬКО там, где указано', безПоля >= 1 && безПоля <= 3,
    '1–3 из 453', безПоля);
  ...
  // Полагаться на состояние, которое никто не подтверждал, значит проверять
  // не то, что думаешь, — поэтому оно ПОДТВЕРЖДАЕТСЯ здесь же. Прежде проба
  // заводила шаг заново поверх заведённого: ход молча снимал шаг и выдавал
  // новый секрет — это и была дыра, закрытая 26.09.2026 (теперь 409).
  const enabledBefore = (await pool.query('SELECT mfa_enabled FROM users WHERE user_id = $1',
    [данныеАвтора.userId])).rows[0].mfa_enabled;
  const reEnroll = await автор.зов('/api/auth/mfa/enroll', { method: 'POST' });
  проверить('повторное заведение заведённого шага по HTTP — 409, шаг остаётся',
    enabledBefore === true && reEnroll.код === 409 && (await pool.query('SELECT mfa_enabled FROM users WHERE user_id = $1',
      [данныеАвтора.userId])).rows[0].mfa_enabled === true, '409; включён', `${enabledBefore}; ${reEnroll.код}`);
  const сноваЗавод = { тело: { data: { секрет: секретАвтора } } };   // секрет, с которым шаг заведён
  ...
  // ── 12-бис. ВОЗВРАТ СУЩНОСТИ К ВЕРСИИ ──────────────────────────────────
  //
  // Откат коммита отвечает «эта правка была ошибкой». Возврат к версии —
  // «верни понятие таким, каким оно было». Разные вопросы, и второго не было:
  // ход GET …/history существовал и не звался НИКЕМ, а вернуться к версии
  // было нечем.
  const историяХ = await рец.зов('/api/graph/concept/harmony_spheres/history');
  проверить('история сущности отдаётся',
    историяХ.код === 200 && Array.isArray(историяХ.тело?.data?.items),
    'массив', `${историяХ.код} ${typeof историяХ.тело?.data?.items}`);
  проверить('в истории есть правки этой сущности',
    (историяХ.тело?.data?.items?.length ?? 0) > 0, '>0',
    историяХ.тело?.data?.items?.length);

  // Правим описание заново, чтобы было куда возвращаться.
  const доВозврата = ((await гость.зов('/api/graph')).тело?.data?.наборы?.concepts ?? [])
    .find(к => к.id === 'harmony_spheres')?.description;
  const версияДо = (await гость.зов('/api/graph')).тело?.data?.версия;
  const свежаяПравка = await рец.зов('/api/commits', { method: 'POST',
    body: { message: 'третья редакция', changes: [{ action: 'edit', kind: 'concept',
      entityId: 'harmony_spheres',
      fields: { description: { base: доВозврата, next: 'редакция третья' } } }] } });
  проверить('правка для возврата легла',
    свежаяПравка.код === 201, 201, свежаяПравка.код);

  const возвратБезПричины = await рец.зов(
    `/api/graph/concept/harmony_spheres/revert-to/${версияДо}`,
    { method: 'POST', body: {} });
  проверить('ВОЗВРАТ БЕЗ ПРИЧИНЫ ОТВЕРГНУТ — возврат тоже правка',
    возвратБезПричины.код >= 400, '4xx', возвратБезПричины.код);

  const возврат = await рец.зов(
    `/api/graph/concept/harmony_spheres/revert-to/${версияДо}`,
    { method: 'POST', body: { reason: 'проба возврата к версии' } });
  проверить('возврат к версии проходит', возврат.код === 200, 200,
    `${возврат.код} ${JSON.stringify(возврат.тело?.error ?? '').slice(0, 70)}`);
  проверить('ВОЗВРАТ — ЭТО КОММИТ, а не тихая правка',
    !!возврат.тело?.data?.commitId, 'есть commitId', возврат.тело?.data?.commitId);

  const послеВозврата = ((await гость.зов('/api/graph')).тело?.data?.наборы?.concepts ?? [])
    .find(к => к.id === 'harmony_spheres')?.description;
  проверить('описание вернулось к тому, что было на той версии',
    послеВозврата === доВозврата, String(доВозврата).slice(0, 40),
    String(послеВозврата).slice(0, 40));

  const впустую = await рец.зов(
    `/api/graph/concept/harmony_spheres/revert-to/${версияДо}`,
    { method: 'POST', body: { reason: 'второй раз' } });
  проверить('повторный возврат к той же версии отвергнут — менять нечего',
    впустую.код >= 400, '4xx', впустую.код);

  // ОТКАЗ НА ЗАВЕДЕНИЕ И УДАЛЕНИЕ В ПРОМЕЖУТКЕ. Если между версией и нынешним
  // состоянием сущность заводили или удаляли, «вернуть к версии» перестаёт
  // значить что-то одно: возвращать ли её из небытия, стирать ли снова.
  // Молчаливый выбор здесь хуже отказа.
  //
  // Случай заведён нарочно: без него отказ не проверялся ничем — подложенная
  // поломка (снять условие вовсе) прошла мимо пробы, и это выяснилось только
  // подлогом, а не чтением.
  const доЗаведения = (await гость.зов('/api/graph')).тело?.data?.версия;
  const завели = await рец.зов('/api/commits', { method: 'POST',
    body: { message: 'новая традиция для пробы возврата',
      changes: [{ action: 'add', kind: 'tradition', entityId: 'проба_возврата',
        fields: { name: { base: null, next: 'Проба' },
                  description: { base: null, next: 'заведена ради проверки' } } }] } });
  проверить('сущность заведена', завели.код === 201, 201, завели.код);
  await рец.зов('/api/commits', { method: 'POST',
    body: { message: 'правка новой традиции',
      changes: [{ action: 'edit', kind: 'tradition', entityId: 'проба_возврата',
        fields: { description: { base: 'заведена ради проверки', next: 'правлена' } } }] } });

  const черезЗаведение = await рец.зов(
    `/api/graph/tradition/проба_возврата/revert-to/${доЗаведения}`,
    { method: 'POST', body: { reason: 'через заведение нельзя' } });
  проверить('ВОЗВРАТ ЧЕРЕЗ ЗАВЕДЕНИЕ ОТВЕРГНУТ, а не угадан',
    черезЗаведение.код >= 400, '4xx', черезЗаведение.код);
  проверить('и сказано, ЧТО ИМЕННО мешает',
    /заводили/.test(JSON.stringify(черезЗаведение.тело?.error ?? '')),
    'заводили', JSON.stringify(черезЗаведение.тело?.error ?? '').slice(0, 60));

  const чужимиРуками = await автор.зов(
    `/api/graph/concept/harmony_spheres/revert-to/${версияДо}`,
    { method: 'POST', body: { reason: 'редактору не дано' } });
  проверить('РЕДАКТОРУ ВОЗВРАТ К ВЕРСИИ НЕ ДАН', чужимиРуками.код >= 400,
    '4xx', чужимиРуками.код);

    // ── 13. ВЫХОД ВЕЗДЕ (ход был, проверки не было) ─────────────────────
  //
  // `POST /api/auth/logout-all` работал и не звался НИКЕМ: ни клиентом, ни
  // пробой, ни инструкцией. Нашлось сличением объявленных маршрутов с их
  // упоминаниями — обратная сторона проверки «служба без хода».
  //
  // ПРОВЕРЯЕТСЯ ОТДЕЛЬНЫМ ЧЕЛОВЕКОМ И В КОНЦЕ. Ход гасит ВСЕ сеансы того,
  // кто его позвал, и всякая проверка после него спрашивала бы у
  // вышедшего. Первые две редакции этого блока сломали соседние разделы —
  // сперва свой, потом чужой.
  const свой = браузер('выходящий везде');
  const регВыход = await свой.зов('/api/auth/register', { method: 'POST',
    body: { username: 'выход_везде', email: 'logout@example.invalid',
            password: ПАРОЛЬ } });
  await pool.query(`UPDATE users SET email_verified_at = NOW() WHERE user_id = $1`,
    [регВыход.тело.data.userId]);
  await свой.зов('/api/auth/login', { method: 'POST',
    body: { email: 'logout@example.invalid', password: ПАРОЛЬ } });

  const другаяВкладка = браузер('его же вторая вкладка');
  await другаяВкладка.зов('/api/auth/login', { method: 'POST',
    body: { email: 'logout@example.invalid', password: ПАРОЛЬ } });
  const ктоТам = await другаяВкладка.зов('/api/users/me');
  проверить('оба сеанса ОДНОГО человека живы до выхода везде',
    ктоТам.код === 200 && ктоТам.тело?.data?.username === 'выход_везде',
    'выход_везде, 200',
    `${ктоТам.код} ${ктоТам.тело?.data?.username} | свой: `
      + JSON.stringify((await свой.зов('/api/users/me')).тело?.data?.username));

  const ответВыхода = await свой.зов('/api/auth/logout-all', { method: 'POST' });

  проверить('ход выхода везде отвечает успехом', ответВыхода.код === 200, 200,
    `${ответВыхода.код} ${JSON.stringify(ответВыхода.тело?.error ?? '').slice(0, 50)}`);
  // СПРАШИВАЕМ У ХОДА, ТРЕБУЮЩЕГО ВХОДА, А НЕ У `/api/users/me`.
  //
  // Тот отдаёт ГОСТЯ со `200` — это правило проекта: гость вправе смотреть
  // граф, и отсутствие входа для него не ошибка. Первая редакция ждала
  // здесь `401` и краснела ЧЕТЫРЕ ЗАХОДА ПОДРЯД, а дефекта не было: в базе
  // все сеансы гасились исправно, и служба, проверенная напрямую, тоже
  // работала. **Красный прибор говорил правду о своём вопросе — вопрос был
  // не тот.**
  const послеВыхода = await другаяВкладка.зов('/api/users/me');
  проверить('после выхода везде другая вкладка стала ГОСТЕМ',
    послеВыхода.тело?.data?.гость === true, 'гость',
    JSON.stringify(послеВыхода.тело?.data).slice(0, 60));
  проверить('ВЫХОД ВЕЗДЕ ГАСИТ И ДРУГУЮ ВКЛАДКУ: правка ей больше не дана',
    (await другаяВкладка.зов('/api/commits', { method: 'POST',
      body: { message: 'после выхода везде', changes: [{ action: 'edit',
        kind: 'concept', entityId: 'logos',
        fields: { description: { base: null, next: 'не должно пройти' } } }] } })).код >= 400,
    '4xx', 'правка прошла');
